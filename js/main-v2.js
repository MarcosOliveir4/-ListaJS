...
function separator() {
  const separatorIconClass =
    list instanceof ListOrdered ? 'arrows alternate horizontal' : 'arrow right';
...
function insertElement() {
  try {
    let value = prompt('Digite um valor a ser inserido:');
    list.append(parseInt(value));
    showData();
  } catch (error) {
    alert(error.message);
  }
...
function removeElement() {
  try {
    let value = prompt('Digite um valor a ser removido:');
    list.remove(parseInt(value));
    showData();
  } catch (error) {
    alert(error.message);
  }
